...
<!-- Left side content -->
  ...
  <!-- Main content wrapper                 -->
    ...
    <form method="post" action="<%=basePath%>sell/listrd.action">
      退货单号:<input type=“text” name="rtnd.srdNo" value="${rtnd.srdNo}"/>
      ...
      <table style="width:100%">
        ...
        <s:iterator value="#request.listrd" var="item">
          <tr>
            <td>${item.srdNo}</td>
            <td>${item.srdGid}</td>
            <td>${item.srdUprice}</td>
            <td>${item.srdNumber}</td>
            <td>${item.srdState}</td>
            <td>
              <a href="<%=basePath%>sell/dtomodifyr.action?rtnd.srdId=${item.srdId}">修改</a>
            </td>
          </tr>
        </s:iterator>
      </table>
        <tr>
      <td style="width:30%; text-align: left"><br><br></td>
        <td>
          <a href="<%=basePath%>sell/listr.action">返回退货单页</a>
        </td>
      </tr>
    </form>
