...
<form method="post" action="<%=basePath%>sell/modifyr.action">
		<table style="width:100%">
			<tr>
				...
				<td>
					<input type="text" name="rtn.srmNo" value="${rtn.srmNo}"/>
				</td>
			</tr>	
			<tr>
				<td style="width: 30%; text-align: right">
				...
				<td>
					<input type="text" name="rtn.srmSum" value="${rtn.srmSum}"/>
				</td>
			</tr>	
			<tr>
				<td style="width: 30%; text-align: right">
				...
				<td>
					<select name="rtn.srmReceive" value="${rtn.srmReceive}">
						<option <s:if test='rtn.srmReceive=="合格"'>selected="selected"</s:if> value="合格">合格</option>
						<option <s:if test='rtn.srmReceive=="不合格"'>selected="selected"</s:if> value="不合格">不合格</option>
						<option <s:if test='rtn.srmReceive=="损坏"'>selected="selected"</s:if> value="损坏">损坏</option>
					</select>
				</td>
			</tr>
			<tr>	
				...
				<td>
					<select name="rtn.srmCname" value="${rtn.srmCname}">
						<s:iterator value="#request.slist" var="item">
							<option value="${item.CName}" <s:if test="rtn.srmCname==#item.CName"> selected="selected"</s:if>>
								${item.CName}
							</option>
						</s:iterator>								
					</select>
			...
			<tr>	
				...
				<td>
					...
					<input id="endDate" name="rtn.srmDate" value="${rtn.srmDate}" readonly="readonly" onfocus="WdatePicker({isShowWeek:true})" />		
				</td>
			</tr>	
			<tr>	
				<td style="width: 30%; text-align: right">
				...
				<td>
					<select name="rtn.srmUser" value="${rtn.srmUser}">
						<s:iterator value="#request.ulist" var="item">
							<option value="${item.UName}" <s:if test="rtn.rmUser==#item.UName"> selected="selected"</s:if>>
								${item.UName}						 
							</option>
						</s:iterator>
					</select>
			...
			<tr>	
				...
				<td>
					<select name="rtn.srmCalculate" value="${rtn.srmCalculate}">
						<option <s:if test='rtn.srmCalculate=="现金"'>selected="selected"</s:if> value="现金">现金</option>
						<option <s:if test='rtn.srmCalculate=="支票"'>selected="selected"</s:if> value="支票">支票</option>
						<option <s:if test='rtn.srmCalculate=="信用卡"'>selected="selected"</s:if> value="信用卡">信用卡</option>						
						<option <s:if test='rtn.srmCalculate=="银行转账"'>selected="selected"</s:if> value="银行转账">银行转账</option>
						<option <s:if test='rtn.srmCalculate=="微信转账"'>selected="selected"</s:if> value="微信转账">微信转账</option>
						<option <s:if test='rtn.srmCalculate=="支付宝转账"'>selected="selected"</s:if> value="支付宝转账">支付宝转账</option>
					</select>
				</td>
			</tr>	
			<tr>	
				...
				<td>
					<select name="rtn.srmState" value="${rtn.srmState}">
						<option value="1">
								正常
						</option>
						<option value="0">
			...
			<tr>
				<td style="width: 30%; text-align: right">
					<input type="hidden" name="rtn.srmId" value="${rtn.srmId}"/>
				</td>
				<td style="width: 30%; text-align: right"><br><br></td>
				<td>
					<input type="submit" value="提交" />
					<a href="<%=basePath%>sell/listr.action">返回</a>
				</td>
			</tr>	
    	</table>
    	</form>
