...
<form method="post" action="<%=basePath%>sell/listr.action">
      退货单号:<input type=“text” name="rtn.srmNo" value="${rtn.srmNo}"/>
      <input type="submit" value="查询"/>
      <a href="<%=basePath%>sell/alistr.action">增加</a>
      <a href="<%=basePath%>sell/listrd.action">订单详情</a>
      <br/>
      <table style="width:100%">
        ...
        <s:iterator value="#request.listr" var="item">
          <tr>
            <td>${item.srmNo}</td>
            <td>${item.srmSum}</td>
            <td>${item.srmReceive}</td>
            <td>${item.srmCname}</td>
            <td>${item.srmDate}</td>
            <td>${item.srmUser}</td>
            <td>${item.srmCalculate}</td>
            <td>${item.srmState}</td>
            <td>
              <a href="<%=basePath%>sell/tomodifyr.action?rtn.srmId=${item.srmId}">修改</a>
            </td>
            <td>
              <a href="<%=basePath%>sell/alistrd.action?rtnd.srmId=${item.srmId}&rtnd.srdNo=${item.srmNo}">添加订单详情</a>
            </td>
          </tr>
        </s:iterator>
      </table>
