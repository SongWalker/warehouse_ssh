...
<form method="post" action="<%=basePath%>sell/list.action">
      销售单号:<input type=“text” name="sell.semNo" value="${sell.semNo}"/>
      <input type="submit" value="查询"/>
      <a href="<%=basePath%>sell/alist.action">增加</a>
      <a href="<%=basePath%>sell/listd.action">订单详情</a>
      <br/>
      <table style="width:100%">
        ...
        <s:iterator value="#request.list" var="item">
          <tr>
            <td>${item.semNo}</td>
            <td>${item.semSum}</td>
            <td>${item.semReceive}</td>
            <td>${item.semSname}</td>
            <td>${item.semDate}</td>
            <td>${item.semUser}</td>
            <td>${item.semCalculate}</td>
            <td>${item.semState}</td>
            <td>
              <a href="<%=basePath%>sell/tomodify.action?store.semId=${item.semId}">修改</a>
            </td>
            <td>
              <a href="<%=basePath%>sell/alistd.action?sd.semId=${item.semId}&sd.sedNo=${item.semNo}">添加订单详情</a>
            </td>
          </tr>
        </s:iterator>
      </table>
