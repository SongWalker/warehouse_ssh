...
<!-- Left side content -->
  ...
  <!-- Main content wrapper                 html 在最后可分为多个html， 这个是进货退货销售单-->
    ...
    <form method="post" action="<%=basePath%>client/list.action">
			   输入条件(客户名/客户ID): <input type="text" name="kehu" size="18" />
        <input type="submit"  value="查询"/>
			<a href="<%=basePath%>inforManage/client/addClient.jsp">添加</a>   
       ...
       <!-- Dynamic table -->
        <div class="widget">
          <div class="title"><img src="images/icons/dark/frames.png" alt="" class="titleIcon" /><h6>客户信息查询</h6></div>
            ...
            <s:iterator value="#request.list" var="item">                                          
                <tr>
                    ...
                    <th>${item.CSname}</th>
                    <th>${item.CAddress}</th>
                    <th>${item.CPostcode}</th>
                    <th>${item.CFax}</th>
                    <th>${item.CContacts}</th>
                    <th>${item.CCtel}</th>
                    <th>${item.CEmail}</th>
                    <th>${item.CBank}</th>
                    <th>${item.CBaccount}</th>
                    <th>${item.CState}</th>
                    <th><a
								href="<%=basePath%>client/tomodify.action?tbc.CId=${item.CId}">编辑</a></th>
                </tr>
            </s:iterator>
                </tbody>
            </table>   
