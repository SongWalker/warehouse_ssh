...
<!-- Left side content -->
  ...
  <!-- Main content wrapper                 html 在最后可分为多个html， 这个是进货退货销售单-->
    <div class="wrapper">
        <form method="post" action="<%=basePath%>customer/list.action">
    
          输入条件(用户名/用户ID): <input type="text" name="yonghu" size="18" />
        <input type="submit"  value="查询"/> 
        <a href="<%=basePath%>inforManage/customer/addCustomer.jsp">添加</a>   
          ...
          <div class="title"><img src="images/icons/dark/frames.png" alt="" class="titleIcon" /><h6>客户信息查询</h6></div>
            ...
            <s:iterator value="#request.list" var="item">                                          <!--方法没写-->
                <tr>
                    ...
                    <th>${item.UAge}</th>
                    <th>${item.UState}</th>
                    <th><a href="<%=basePath%>customer/tomodify.action?user.UId=${item.UId}">编辑</a></th>
                </tr>
            </s:iterator>
                </tbody>
            </table>   
