...
<!-- Left side content -->
  ...
  <!-- Main content wrapper                 html 在最后可分为多个html， 这个是进货退货销售单-->
      <form method="post" action="<%=basePath%>supplier/list.action">
    <div class="wrapper">   
输入条件(供应商姓名/供应商ID):
 <input type="text" name="gys" size="18" />
			<input type="submit" value="查询" />
            <a href="<%=basePath%>inforManage/supplier/addSupplier.jsp">添加</a>   
            <a href="<%=basePath%>supplier/list.action">返回</a>
            <br />

       <!-- Dynamic table -->
        <div class="widget">
          <div class="title"><img src="images/icons/dark/frames.png" alt="" class="titleIcon" /><h6>供应商信息查询</h6></div>
            ...
            <s:iterator value="#request.list" var="item">                                          <!--方法没写-->
                <tr>
                    ...
                    <th>${item.SSname}</th>
                    <th>${item.SAddress}</th>
                    <th>${item.SPostcode}</th>
                    <th>${item.STel}</th>
                    <th>${item.SFax}</th>
                    <th>${item.SContacts}</th>
                    <th>${item.SCtel}</th>
                    <th>${item.SBank}</th>
                    <th>${item.SEmail}</th>
                    <th>${item.SState}</th>
                    <th>
                        <a href="<%=basePath%>supplier/tomodify.action?tbsup.SId=${item.SId}">编辑</a>
                    </th>
                </tr>
            </s:iterator>
                </tbody>
