...
<form method="post" action="<%=basePath%>store/list.action">
      入库单号:<input type=“text” name="store.smNo" value="${store.smNo}"/>
      <input type="submit" value="查询"/>
      <a href="<%=basePath%>store/alist.action">增加</a>
      <a href="<%=basePath%>store/listd.action">订单详情</a>
      <br/>
      <table style="width:100%">
        ...
        <s:iterator value="#request.list" var="item">
          <tr>
            <td>${item.smNo}</td>
            <td>${item.smSum}</td>
            <td>${item.smReceive}</td>
            <td>${item.smSname}</td>
            <td>${item.smStdate}</td>
            <td>${item.smUser}</td>
            <td>${item.smCalculate}</td>
            <td>${item.smState}</td>
            <td>
